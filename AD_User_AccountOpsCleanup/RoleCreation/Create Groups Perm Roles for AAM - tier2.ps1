...
$permissionset = .($scriptPath + "\AD Permissions for Group Granular Access - Groups AAM.ps1")
#=====================================
...
#Tier 2
$Tier1GroupLocation = "OU=T2-Permissions,OU=Tier 2,OU=Admin" + "," + $dn
cd ad:
...
#BEGIN MAKING GROUPS AND SETTING ACLS
   ...
   # Write-Progress -Activity "Deploying OU Structure" -Status "Affiliate Permissions Set Deploy Status:" -PercentComplete ($x/$CSVCount*100)
    ...
    foreach ($permission in $permissionset){
        ...
        $t1groupname = ("AAM_T2_" +($permission.FunctionSet.Split( ))[0] +"_" + $permission.FunctionName)
