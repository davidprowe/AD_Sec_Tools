...
$OUArray = @(("OU=IT Administration,"+ $DN),`
("OU=Quarantine,"+ $DN),`
("OU=Servers,OU=Technology,"+ $DN),`
("OU=Server New Builds,OU=!Pre-Production,"+ $DN)
)
...
$permissionset = .($scriptPath + "\AD Permissions for Group Granular Access - LAPS.ps1")
#=====================================
...
#BEGIN MAKING GROUPS AND SETTING ACLS
   ...
   # Write-Progress -Activity "Deploying OU Structure" -Status "Affiliate Permissions Set Deploy Status:" -PercentComplete ($x/$CSVCount*100)
    ...
    foreach ($permission in $permissionset){
        ...
        $t1groupname = "IT-AD-LAPS-FOR-SERVERS-ACCESS"
